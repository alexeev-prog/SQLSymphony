...
class ChangeObserver(ABC):
	...
	@abstractmethod
	async def on_change(self, audit_entry: "AuditEntry"):
		"""
		Called on change.
...
class AuditSubject(ABC):
	...
	@abstractmethod
	async def attach(self, observer: ChangeObserver):
		"""
		Attach observer
	...
	@abstractmethod
	async def detach(self, observer: ChangeObserver):
		"""
		Detach observer
	...
	@abstractmethod
	async def notify_observers(self, audit_entry: AuditEntry):
		"""
		Notifies observers.
...
class AuditStorage(ABC):
	...
	@abstractmethod
	async def save_audit_entry(self, audit_entry: AuditEntry):
		"""
...
class AuditManager(AuditSubject):
	...
	async def attach(self, observer: ChangeObserver):
		...
		:type		observer:  ChangeObserver
		"""
		await self.observers.append(observer)
	...
	async def detach(self, observer: ChangeObserver):
		...
		:type		observer:  ChangeObserver
		"""
		await self.observers.remove(observer)
	...
	async def track_changes(
		...
		await self.audit_storage.save_audit_entry(audit_entry)
		await self.notify_observers(audit_entry)
	...
	async def get_audit_history(
		...
		:rtype:		List[AuditEntry]
		"""
		result = await self.audit_storage.get_audit_history(
			model_name, table_name, object_id
		)
		return result
