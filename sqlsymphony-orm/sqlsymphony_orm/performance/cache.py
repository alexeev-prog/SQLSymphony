...
class CacheBase(object):
	...
	def set(self, key: str, value: Any, timestamp: float) -> None:
		...
		Args:
				...
				timestamp (float): The timestamp when the value was generated.
		"""
		raise NotImplementedError
...
class CacheFactory(object):
	...
	def create_cache(cache_type: Type[CacheBase], *args, **kwargs) -> CacheBase:
		...
		Args:
				cache_type (Type[CacheBase]): The type of cache to create.
				*args: Positional arguments to pass to the cache constructor.
				**kwargs: Keyword arguments to pass to the cache constructor.

		Returns:
				CacheBase: A new instance of the specified cache type.
		"""
...
def cached(
	...
	results of the decorated function or method. The key_func argument allows you to
	customize how the cache key is generated from the function/method arguments.

	Args:
			cache (SingletonCache): The cache instance to use for caching.
			key_func (Callable[[Any, Any], str]): A function that generates the cache
					key from the function/method arguments.

	Returns:
			Callable: A new function or method that caches the results.
	"""
