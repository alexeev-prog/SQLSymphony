...
async def distribute(items, num_containers, hash_function=hash):
	"""
...
class PlainHasher(HashingBase):
	...
	async def hash(self, data: Union[bytes, str]) -> bytes:
		"""
		Generate hash
	...
	async def verify(self, data: Union[bytes, str], hashed_data: Union[bytes, str]) -> bool:
		...
		expected_hash = await self.hash(data)

		result = await compare_digest(expected_hash, hashed_data)
		return result

	async def get_hasher(self) -> callable:
		"""
...
class SaltedHasher(HashingBase):
	...
	async def hash(self, data: Union[bytes, str]) -> bytes:
		...
		result = hasher(value).digest()
		return result

	async def verify(self, data: str, hashed_data: Union[bytes, str]) -> bool:
		"""
		...
		result = compare_digest(expected_hash, hashed_data)
		return result
